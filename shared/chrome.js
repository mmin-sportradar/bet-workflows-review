...
(function () {
  ...
  function buildFooter() {
      ...
      <div class="sr-footer-actions">
        <div class="sr-feedback-slot">
          ...
          <form class="sr-feedback-comment" hidden>
            ...
            <input id="sr-feedback-name" class="sr-feedback-name-input" type="text"
              placeholder="Your name (so we can come back to you)"
    ...
    const who = card.querySelector(".sr-feedback-name-input");

    // Remembered the way the team switcher is, so nobody types their name twice.
    // Optional, always: a name makes a comment answerable, and demanding one is
    // how you stop getting comments at all.
    const NAME_KEY = "bet-workflows:name";
    try {
      const saved = localStorage.getItem(NAME_KEY);
      if (saved && who) who.value = saved;
    } catch (err) {
      /* private mode -- they can type it */
    }
    ...
    function finish() {
      ...
      //
      // And it says which of the two actually happened. "Thanks for the
      // feedback" was shown whether or not there was anywhere to send it, so
      // somebody left a comment on a site with no sink configured, was thanked
      // for it, and reasonably went looking for it in Slack. The feedback was
      // not lost -- it is queued in this browser and drains when a sink is
      // connected -- but nothing said so.
      const sent = !window.betFeedback || window.betFeedback.configured();
      thanks.textContent = sent
        ? "Thanks for the feedback."
        : "Thanks — saved here, and sent once feedback is connected.";
    }
    ...
    function submit(comment) {
      const name = who ? who.value.trim().slice(0, 80) : "";
      if (name) {
        try {
      ...
      if (window.betFeedback) window.betFeedback.submit(rating, comment, name);
      finish();
    }
  ...
  function loadAssistant(host) {
    ...
    styles.href = `${root}shared/assistant.css?v=bbc8f30892`;
    ...
    feedback.src = `${root}shared/feedback.js?v=bbc8f30892`;
    ...
    onyx.src = `${root}shared/onyx.js?v=bbc8f30892`;
    document.head.append(onyx);

    const knowledge = document.createElement("script");
    knowledge.src = `${root}shared/knowledge.js?v=bbc8f30892`;
    knowledge.addEventListener("load", () => {
      const widget = document.createElement("script");
      widget.src = `${root}shared/assistant.js?v=bbc8f30892`;
