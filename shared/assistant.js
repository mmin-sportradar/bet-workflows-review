...
(function () {
  ...
  const ALIASES = {
    ...
    "us widgets": "US Widgets",
    "uswidgets": "US Widgets",
    "sps widgets": "US Widgets",
    "sports page solutions": "US Widgets",
    // Names readers type that the catalogue does not spell. Probing 112
    // plausible forms found 40 that resolved nothing; these are the ones with a
    // single unambiguous owner. A one-word alias has to clear VAGUE and three
    // characters (see register), which is why "widgets" cannot be one and
    // "bet widgets" is.
    "bet widgets": "Betting Widgets",
    "betting widget": "Betting Widgets",
    "prop zone": "Player Prop Zone",
    "concierge": "Bet Concierge",
    "stat hub": "StatsHub",
    "vlmt": "LMT Virtualised",
    "v lmt": "LMT Virtualised",
    "lmt virtual": "LMT Virtualised",

    // Reversed word order, for the family where readers actually reverse it.
    // resolveMentions scans n-grams in order, so "premium lmt" misses where
    // "lmt premium" hits. Reversing generally was measured over 1197 strings:
    // seven gains and one loss -- the thread "Golf LMT Premium package
    // required" stops resolving LMT Premium, because "golf lmt" reverses onto
    // the mined key "lmt golf" and greedily eats both words. Three aliases buy
    // the gains without the loss.
    "premium lmt": "LMT Premium",
    "plus lmt": "LMT Plus",
    "virtualised lmt": "LMT Virtualised"
  };
  ...
  //
  // The -ate/-ation pair is here for one reason: someone asking how to
  // "integrate" a widget wants the thousand passages that say "integration".
  // The prefix fallback cannot bridge those two, because they part company at
  // the ninth character. It is deliberately this one pair rather than a general
  // suffix stripper -- this is the ranking function for the whole corpus, and
  // every extra rule is another chance to merge two words that should stay
  // apart.
  function stem(word) {
    if (word.length > 5 && word.endsWith("ing")) return word.slice(0, -3);
    if (word.length > 4 && word.endsWith("es")) return word.slice(0, -2);
    if (word.length > 3 && word.endsWith("s") && !word.endsWith("ss")) word = word.slice(0, -1);
    if (word.length > 6 && word.endsWith("ation")) return word.slice(0, -3);
    if (word.length > 5 && word.endsWith("ate")) return word.slice(0, -1);
    return word;
  }
  ...
  function loadDocs() {
    ...
    docsState.promise = new Promise((resolve) => {
      ...
      script.src = `${root}shared/docs-knowledge.js?v=ffea976122`;
  ...
  function search(query, channel, resolved) {
    ...
    // KNOWN BUG, left alone deliberately. This never runs: search() is called
    // with an entities ARRAY, so `resolved.ambiguous` is undefined and an
    // ambiguous surface's own words stay in the topic. Fixing it corrects
    // topicCoverage, which moves the docs-versus-thread comparisons at the
    // `defining` clauses below -- measured, "lmt 504 timeout" stopped answering
    // with its own thread and started answering with a status-code table. That
    // is a real fix and a real risk, and it belongs in its own change with its
    // own measurement rather than riding along with a recall fix.
  ...
  function coversResolved(entry, resolved) {
    ...
    const declared = entitiesOf(entry);
    // A general note is about everything, and so -- it turns out -- is an
    // untagged incident report. 444 of the thousand-odd DOCS passages come from
    // sections owned by no single product, and treating those as covering every
    // product is the bug this guard exists for, at six times the scale. That
    // argument is about documentation, and the docs branch below is unchanged.
    //
    // For a thread it was measured wrong. Sixteen of the sixty-five threads
    // were unreachable by their own words because of this line: ALIASES maps
    // "whitelisting" onto the Additional domains product, so the question
    // resolved a subject the thread compiler never tags -- productsIn() matches
    // catalogue labels literally and knows nothing about aliases -- and every
    // one of the sixteen was the TOP hit at coverage 1.00 and refused anyway.
    // The entity veto still applies (x0.4 below), so an untagged thread has to
    // out-score a tagged one four to one to win.
    if (!declared.length) {
  ...
  // Two bars on the same measure -- the rarity mass of the words that are the
  // question rather than the product's name -- for the two ways an ambiguous
  // question can be answered instead of asked back.
  //
  // MIN is "there is a topic here at all", and replaces a count of topical
  // words. One word was enough, and "which lmt should I use" leaves "should",
  // which let a shared documentation passage rescue a question that was
  // literally asking which of the four. Measured: "which lmt should I use"
  // 2.98 and "lmt help" 3.68 must not pass; "how do you set client id on LMT"
  // 4.83 and "how do I reorder the lmt tabs" 8.39 are eval questions that must.
  //
  // WRITTEN is the higher bar for handing over one product's answer, where
  // being wrong is worse: "tell me about lmt" reaches 5.16 and still deserves
  // the question back, while every question that deserves an answer measured
  // 8.81 or more.
  const AMBIGUOUS_TOPIC_MIN = 4;
  const AMBIGUOUS_TOPIC_IDF = 7;
  ...
  const state = {
    ...
    history: [],
    // The options from a "which one did you mean?", so a typed reply can be
    // read as the answer to it. Null the rest of the time.
    pending: null,
    // The id of the last entry given, so a follow-up that inherits its topic is
    // not answered with the paragraph the reader has just read.
    answered: null,
    open: false
  };
  ...
  function wirePanel() {
    ...
    panel.querySelector(".sr-ask-body").addEventListener("click", (event) => {
      ...
      // An option carries the question to ask rather than an entry id: the
      // question is the original one with the product named in it, which no
      // stored entry corresponds to.
      const choice = event.target.closest(".sr-ask-chip[data-ask]");
      if (choice) {
        ask(choice.dataset.ask);
        return;
      }
  ...
  function derived(id, question, answer, links, related, entities, choices) {
    const entry = {
      id,
      question,
      answer,
      // Options for a question the assistant itself asked. Kept apart from
      // `related`, which means "People also asked" -- these are the answer to
      // what it just said, not a tangent.
  ...
  // Put the chosen name into the question that was asked, so picking an option
  // answers what somebody actually wanted. "how do I set up LMT" becomes "how
  // do I set up LMT Premium" -- not "what is LMT Premium", which is what the
  // options used to offer and is rarely the question.
  const escapeRegExp = (value) => String(value).replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  function pinTo(question, surface, label) {
    const at = new RegExp(`\\b${escapeRegExp(surface)}\\b`, "i");
    return at.test(question) ? question.replace(at, label) : `${question} ${label}`;
  }

  // A correct answer to an underspecified question, not a miss -- so it renders
  // like any other answer, with the candidates as buttons.
  //
  // They used to be passed as `related` ids of the form derived:what-is:<id>,
  // and not one ever rendered: those entries are created only when a "what is"
  // answer is composed, which happens later in arbitrate than this does, so at
  // this moment none of them exist and every option was dropped as unresolvable.
  // What the reader saw was "Which one did you mean?" with nothing to click.
  function composeAmbiguous(mention, question) {
    const candidates = mention.candidates;
    const vague = new Set(normalise(mention.text).split(" "));

    // Pinning the name into the question only works when the question carries
    // one. "how do I set up LMT" becomes "how do I set up LMT Premium", which
    // is what was wanted. "which lmt should I use" becomes "which LMT Premium
    // should I use", which is neither English nor a question -- and there the
    // product's own profile is the answer anyway. A word count is enough here:
    // this only chooses the wording of a button, and unlike the rescue gate
    // above nothing is hidden if it reads the question slightly differently.
    const topical = contentWords(tokenize(question || "")).filter(
      (word) => !vague.has(word) && weigh(word) === 1
    );
    const pin = (label) =>
      topical.length >= 2 ? pinTo(question, mention.text, label) : `What is ${label}?`;

    const choices = candidates.map((entity) => ({
      label: entity.label,
      ask: pin(entity.label),
      // The words that tell the options apart -- premium, virtualised -- taken
      // from the labels rather than written out a second time, so a new product
      // in the catalogue needs no change here.
      keys: normalise(entity.label).split(" ")
        .filter((word) => word && !vague.has(word) && weigh(word) === 1),
    }));
    return derived(
  ...
  // "the second one", "2", "the last".
  function ordinalIn(reply, count) {
    const text = normalise(reply);
    if (/\blast\b/.test(text)) return count - 1;
    const digit = text.match(/\b([1-9])\b/);
    if (digit && Number(digit[1]) <= count) return Number(digit[1]) - 1;
    for (let i = 0; i < ORDINALS.length && i < count; i++) {
      if (new RegExp(`\\b${ORDINALS[i]}\\b`).test(text)) return i;
    }
    return null;
  }

  // Read a short reply as the answer to the question the assistant just asked.
  // Deliberately narrow: answering the wrong product is worse than treating the
  // reply as a new question, so anything unclear falls through.
  function pickPending(pending, reply) {
    const words = new Set(normalise(reply).split(" ").filter(Boolean));
    // A sentence is a new question, not a choice.
    if (!words.size || words.size > 4) return null;

    const named = pending.choices.filter((choice) =>
      (choice.keys || []).some((key) => words.has(key))
    );
    if (named.length === 1) return named[0].ask;
    // Two options named, or none: not an answer to the question.
    if (named.length > 1) return null;

    const nth = ordinalIn(reply, pending.choices.length);
    return nth === null ? null : pending.choices[nth].ask;
  }
  ...
  const ANAPHOR = /\b(it|its|it's|that|this|these|those|them|they|one|the same)\b/i;

  // A follow-up that names nothing is a question about whatever was just being
  // discussed. On its own words it is close to meaningless -- "how do I
  // integrate it?" has one content word, so anything containing that word
  // accounts for 100% of the question and wins -- and "how do I use it" scored
  // nothing at all. Both read as the assistant having no memory, which is the
  // thing people notice first about it.
  //
  // Narrow on purpose, and in the same spirit as the clarification reply above:
  // one turn deep, and only where the follow-up genuinely cannot stand alone.
  // It has to resolve nothing itself AND either point at something with a
  // pronoun or carry no subject matter of its own; the question before it has to
  // resolve a product. A second question about a different thing is not a
  // follow-up, and inheriting a subject it did not ask about would be worse than
  // having no memory.
  function carryTopic(question, resolved) {
    if (subjectsOf(resolved.entities).length || resolved.ambiguous.length) return null;
    if (!ANAPHOR.test(question) && contentWords(tokenize(question)).length > 1) return null;

    // The current question is already on the history by the time this runs.
    const previous = state.history[state.history.length - 2];
    if (!previous) return null;

    // The previous question's words join the search, so the ranking sees the
    // topic; the follow-up's own words are what classify() and arbitrate() read,
    // because the SHAPE of the question is the follow-up's own -- taking the
    // shape from the earlier one would answer "how do I integrate it" as though
    // it were still "who owns LMT Plus".
    //
    // Its ENTITIES come too, but only if it resolved any. Half the topics people
    // follow up on are not products at all -- a client id, a JWT, an adapter --
    // and those carry perfectly well as words. "What is client ID?" followed by
    // "how do I integrate it?" is the sequence this was reported from.
    const before = resolveMentions(previous);
    return {
      resolved: subjectsOf(before.entities).length ? before : resolved,
      scope: `${question} ${previous}`,
    };
  }
  ...
  function answer(question, forced) {
    ...
    // A reply to "which one did you mean?" is about the question before it, not
    // a question of its own -- "premium" resolves to nothing on its own, so
    // without this it searched the whole corpus for the word and found whatever
    // happened to contain it. One turn deep on purpose: this is a clarification,
    // not a conversation history.
    if (state.pending) {
      const pinned = pickPending(state.pending, question);
      state.pending = null;
      if (pinned) question = pinned;
    }

    let resolved = resolveMentions(question);
    let scope = question;
    const carried = carryTopic(question, resolved);
    if (carried) {
      resolved = carried.resolved;
      scope = carried.scope;
    }
    ...
    warmFlows(resolved);
    const hits = search(scope, state.channel, resolved.entities);
    let chosen = arbitrate(question, resolved, intent, hits);

    // A follow-up borrows the previous question's words, so the entry that
    // answered THAT question is now the best match for this one too: "what is
    // client ID?" and then "how do I integrate it?" both came back with the same
    // paragraph, which reads as the assistant not having heard the second
    // question. Asked something new, answer with something new -- and only where
    // the topic was inherited, so this can never displace a repeat somebody
    // typed out deliberately.
    if (carried && chosen && chosen.id === state.answered) {
      const other = arbitrate(
        question,
        resolved,
        intent,
        hits.filter((hit) => hit.entry.id !== chosen.id)
      );
    ...
    // Remember an unanswered question so the next reply can be read against it.
    state.pending = chosen && chosen.choices && chosen.choices.length
      ? { choices: chosen.choices }
      : null;

    state.answered = chosen.id;
    appendMessage("bot", answerHtml(chosen, hits.filter((hit) => hit.entry !== chosen).slice(0, 2), resolved));
  }
  ...
  function pickPassage(eligible, subjects) {
    ...
    // And with a product named, the passage from THAT product's page.
    //
    // Several pages carry the same section word for word -- the property-name
    // transformations are identical on LMT Compact, LMT Plus, LMT Golf and LMT
    // eSports, and all four declare the whole LMT family. So they score within a
    // hair of each other, the ranking's tie-break falls through to answer length,
    // and "how do I set the client id on LMT Plus" was quoted the LMT COMPACT
    // page. The words are right and the citation says the wrong product, which
    // is worse than a wrong answer: it is a right one the reader cannot trust.
    //
    // Only among passages that already fit about equally well, so this cannot
    // reach past a better answer to a worse one on its page title.
    if ((subjects || []).length) {
      const close = pool.filter((hit) => hit.score >= best.score * 0.85);
      if (close.length > 1) {
        const named = close.filter((hit) => {
          const page = new Set(normalise(hit.entry.pageTitle || "").split(" "));
          return subjects.some((entity) =>
            normalise(entity.label).split(" ").every((word) => page.has(word))
          );
        });
  ...
  function arbitrate(question, resolved, intent, hits) {
    ...
    if (resolved.ambiguous.length && !subjectsOf(resolved.entities).length) {
      ...
      //
      // topicIdf is that measure and search() computes it already, with the
      // ambiguous surface's own words taken out, so the name cannot make the
      // question look bigger than it is. That subtraction was the thing this
      // used to do by hand and is why the comment here used to talk about
      // labels and lengths.
      // The rarity mass of the words that are the question rather than the
      // product's name. Computed here rather than taken from the hit, because
      // the hit's topicIdf is subject to the bug recorded in search() and still
      // counts the ambiguous name as part of the topic.
      const vague = new Set(normalise(resolved.ambiguous[0].text || "").split(" "));
      const topicMass = contentWords(tokenize(question))
        .filter((word) => !vague.has(word) && !vague.has(stem(word)))
        .reduce((sum, word) => sum + idf(word) * weigh(word), 0);

      const asking = topicMass >= AMBIGUOUS_TOPIC_MIN;
      ...
      // A written answer could not rescue an ambiguity at all, however well it
      // fitted -- only a shared docs passage could. The reasoning above is that
      // docs are product-neutral, but a thread somebody wrote about the exact
      // symptom is better evidence of "the answer is the same whichever they
      // meant", not worse: it is about the symptom, not the product.
      //
      // "lmt 504 timeout" found its own thread at full coverage and was still
      // told "lmt could be 4 things". Seven of the sixty-five threads were
      // unreachable that way -- every one an LMT question, and in every one the
      // right thread was already the top hit.
      //
      // Only the TOP hit may rescue, and only if it clears its own confidence
      // bar. Searching the whole list for any confident written answer was too
      // generous: "how do I set up LMT" scored two docs passages highest and
      // then found a general entry about picking a product tile further down,
      // so a question that genuinely does need "which of the four?" was
      // answered "open the catalogue and pick the tile". If the single best
      // answer to the question is not a written one about it, ask.
      //
      // The `asking` gate above still protects a bare "lmt": no topic at all,
      // so "which of the four?" stays the right answer there.
      // Enough of a topic to be worth answering whichever product was meant.
      //
      // This counted topical WORDS first, and two of them. That threshold was
      // calibrated against the thread summaries, which are titles carrying
      // three to eight -- so it never sampled its own boundary, while every
      // reader-typed question sits exactly on it and one word drifting into
      // DEMOTED_WORDS would flip it. Rarity mass is the same measure without
      // the cliff: a demoted word contributes a quarter of its idf rather than
      // nothing, and the figure does not move when the wording gets longer.
      //
      // Measured on the corpus, the two populations do not overlap. Questions
      // that deserve an answer: 8.81 (pitchview), 9.54 (404 client id), 9.91,
      // 9.98 (tennis licence), 10.8 (504 timeout), 12.64, 14, 18.96, 22.35.
      // Questions that deserve "which one did you mean?": 0 (bare "lmt", "what
      // is lmt", "packages"), 0.87 ("list the lmt products"), 2.98 ("which lmt
      // should I use"), 3.68 ("lmt help"), 5.16 ("tell me about lmt"). Seven
      // sits in the middle of an empty gap.
      const written = asking && topicMass >= AMBIGUOUS_TOPIC_IDF &&
        !shared && hits.length && !isDocs(hits[0].entry) &&
        curatedIsConfident(hits[0], resolved.entities, question, hits[0])
        ? hits[0]
        : null;

      if (!shared && !written) return composeAmbiguous(resolved.ambiguous[0], question);
    }
    ...
    // Whether the hand-written answer WON the curated stratum or was lifted into
    // it by the band above. The band says "these two fit about equally well, and
    // between equals the authored one wins" -- a judgement made among curated
    // answers, about which of them to give. It is not evidence that the entry is
    // the best answer in the corpus, and the rules below read it as though it
    // were: every one of them steps aside for a site entry, so an authored note
    // promoted from 62% of the top score then took absolute precedence over a
    // passage scoring half again as much.
    //
    // "how to integrate client id?" was answered with the note on setup flows
    // versus integration flows, which says nothing about client ids and matched
    // on "integration", "the client" and "ID mapping" in three unrelated
    // sentences. The FAQ section titled Client ID Authentication -- the same one
    // that answers "what is client ID?" -- scored 14.92 against its 9.78 and was
    // never reached.
    const promoted = !!general && general.entry.source === "site" && general !== curated[0];
    ...
    if (!bareName && curatedIsConfident(general, resolved.entities, question, hits[0])) {
      ...
      const outscored =
        (general.entry.source !== "site" || promoted) &&
        passage &&
    ...
    // 8. The product is known but the question is not. Name the gap -- unless
    //    the best hit is a thread about exactly this, which is not a gap.
    //
    //    "I don't have anything on that specific question" in front of an
    //    answer we do have is the dead end, not the honesty. Narrow on purpose:
    //    it can only fire where composeGap was about to, so it cannot displace
    //    anything that already wins.
    const subjects = subjectsOf(resolved.entities);
    if (subjects.length) {
      if (best && best.entry.source === "thread" && best.coverage >= 0.9) return best.entry;
      return composeGap(subjects[0], hits, question);
    }
  ...
  // Products by their catalogue label, for entries that name one in words
  // rather than by id. Documentation passages and threads both do.
  const byLabel = new Map(NAMEABLE.map((entity) => [normalise(entity.label), entity]));

  // The internal page for whatever the answer is about, offered beside it.
  //
  // A quoted passage is the PUBLIC documentation: it says what the product does
  // and never what this company does with it -- who provisions it, which team
  // owns which step, what was agreed with which client. Confluence holds that
  // half, and the two together are the answer to most real questions.
  //
  // A link and only ever a link. Nothing from Confluence is copied into this
  // corpus, deliberately: shared/** is force-pushed to a public repository
  // served without a login, the ingester excludes those pages by name, and the
  // publish workflow fails the build on any passage whose text mentions one. A
  // URL asserts nothing and leaks nothing; the reader either has access or does
  // not.
  function confluenceFor(entry, resolved) {
    // What the READER named comes first -- they have already said which product
    // they are asking about.
    let entities = subjectsOf((resolved && resolved.entities) || []);

    // Failing that, what the ENTRY names, and only when it names exactly one.
    // Four LMT pages declare the whole family, and hanging four internal pages
    // off an answer about none of them in particular is noise, not help.
    if (!entities.length && (entry.entities || []).length === 1) {
      const found = byLabel.get(normalise(entry.entities[0]));
      if (found) entities = [found];
    }
    ...
    entities.forEach((entity) =>
      (entity.confluence || []).forEach((page) => {
        if (!page || !page.href || seen.has(page.href)) return;
        seen.add(page.href);
        // Say whose page it is. Most of these are titled "Confluence
        // Documentation" by default, which beside "Read this in the
        // documentation" reads as a second copy of the same link rather than
        // as the internal page for one product. Titles that already name the
        // product are left alone -- the catalogue's own wording wins.
        const names = normalise(page.title || "").includes(normalise(entity.label));
        links.push(names ? page : { title: `${entity.label} — ${page.title}`, href: page.href });
      })
    );
    return links;
  }

  function answerHtml(entry, alternatives, resolved) {
    ...
    if (entry.source === "docs") {
      // The page and section used to appear twice -- once as a heading above the
      // quote and again as the link's own label -- which is the same string
      // shouted and then repeated. The quote and its link are the answer; where
      // it came from belongs on the link and in the provenance line, once.
      const link = {
        title: "Read this in the documentation",
        href: entry.href,
        strong: true,
      };
      ...
      return `
        ...
        ${linksHtml([link, ...confluenceFor(entry, resolved)])}
        <div class="sr-ask-source">${icon.external}<span class="sr-ask-source-text">Quoted from ${
          where ? `<b>${escapeHtml(where)}</b>, ` : ""
        }on the public ${escapeHtml(entry.site || "product documentation")}${
          entry.updatedAt ? `, updated ${escapeHtml(entry.updatedAt)}` : ""
        }.</span></div>
        ${rateHtml()}
      `;
    }
    ...
    if (entry.source === "thread") {
      return `
        ...
        ${linksHtml([...(entry.links || []), ...confluenceFor(entry, resolved)])}
  ...
  // The options for a question the assistant asked. Deliberately not under
  // "People also asked": these answer what it just said, and filing them as
  // tangents is part of why the disambiguation read as a dead end.
  function choicesHtml(choices) {
    if (!(choices || []).length) return "";
    return `<div class="sr-ask-followups">
        <p class="sr-ask-followups-label">Which one did you mean?</p>
        <div class="sr-ask-chips">${choices
          .map(
            (choice) =>
              `<button type="button" class="sr-ask-chip sr-ask-choice" data-ask="${escapeHtml(
                choice.ask
  ...
  function derivedHtml(entry, alternatives) {
    ...
    // An answer that asks a question offers its own options, and nothing else.
    // Runner-up hits beside them is what the reader was actually getting: two
    // chips about team filters underneath "lmt could be 4 things". No .slice
    // either -- there are four LMT products and the cap of three silently
    // dropped one of the answers to the question being asked.
    const followupHtml = entry.choices && entry.choices.length
      ? choicesHtml(entry.choices)
  ...
  function restart() {
    threadEl.innerHTML = "";
    state.pending = null;
    // The channel filter outlived the conversation it was chosen for, so a
    // fresh start silently kept searching one channel.
    state.channel = null;
  ...
  function goBack() {
    ...
    // Stepping back off an unanswered question un-asks it.
    state.pending = null;
  ...
  window.betAssistant = {
    ...
    // Arbitration was unreachable from a test, which is the mechanical reason
    // no suite could see that a quarter of the corpus was unanswerable: the
    // ranker put every one of those threads at rank 0 with full coverage, and
    // arbitrate threw them away. scorer.mjs asserts the ranker; nothing
    // asserted this.
    __arbitrate: (question, resolved, intent, hits) =>
      arbitrate(question, resolved, intent, hits)
  };
})();
